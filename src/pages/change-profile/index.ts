...
export function changeProfileProps(user?: IUserModel) {
  ...
  let FormElementEmail = new FormElement({
    ...
    events: {
      blur: function () {
        ...
        if (error) {
          error.hidden =
            emailValidate(inputEmailValue) || inputEmailValue === "";
        }
      },
    },
  });
  ...
  let FormElementLogin = new FormElement({
    ...
    events: {
      blur: function () {
        ...
        if (error) {
          error.hidden =
            loginValidate(inputLoginValue) || inputLoginValue === "";
        }
      },
    },
  });
  ...
  let FormElementFirstName = new FormElement({
    ...
    events: {
      blur: function () {
        ...
        if (error) {
          error.hidden =
            firstNameValidate(inputFirstNameValue) ||
            inputFirstNameValue === "";
        }
      },
    },
  ...
  let FormElementNickName = new FormElement({
    ...
    classNameInput: "form__input",
    spanId: "nickName-input-error",
    errorText: "Некорректное имя",
    events: {
      blur: function () {
        const error = document.getElementById("nickName-input-error");
        ...
        if (error) {
          error.hidden =
            displayNameValidate(inputNickNameValue) ||
            inputNickNameValue === "";
        }
      },
    },
  ...
  let FormElementPhone = new FormElement({
    ...
    events: {
      blur: function () {
        ...
        if (error) {
          error.hidden =
            phoneValidate(inputPhoneValue) || inputPhoneValue === "";
        }
      },
    },
  });
    ...
    events: {
      click: function (e: Event) {
        ...
        if (
          ...
          userController.changeUserProfile(data as IProfile).then((res) => {
            if (res.status === 200) {
              router.go("/settings");
            } else {
